...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, RouterLink, FormsModule, AddItemFormComponent],
...
export class ProductListComponent {
  ...
  load() {
    this.api.getProducts(this.invoiceId).subscribe(list => this.products = list);
  }

  get assignedCount(): number {
    return this.products.filter(p => p.status === 'assigned' || p.status === 'picked' || p.status === 'dispatched' || p.status === 'received').length;
  }
  ...
  triggerBulkUpload() {
    ...
    this.fileInput.onchange = () => this.handleBulkFile(this.fileInput!.files![0]);
  ...
  downloadSample() {
    const headers = 'Brand,Product Name,Model,IMEI,Colour,RAM,Storage,HSN Code,SKU,Barcode';
    const sample = 'Samsung,Galaxy A54,SM-A546,351234567890123,Black,8 GB,128 GB,8517,SAM-A54-BLK-128,8901234567890';
    const csv = headers + '\n' + sample;
